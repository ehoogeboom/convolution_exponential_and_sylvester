...
class MaskedSpectralNormConv(object):
    ...
    def __init__(self, coeff, input_dim, name='weight', maskname=None,
    ...
    def compute_weight(self, module, do_power_iteration):
        ...
        mask = getattr(module, self.maskname)
    ...
    @staticmethod
    def apply(module, coeff, input_dim, name, maskname, n_power_iterations, eps):
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNormConv) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                   "the same parameter {}".format(name))

        fn = MaskedSpectralNormConv(coeff, input_dim, name, maskname, n_power_iterations, eps)
        weight_ = module._parameters[name]
        mask = getattr(module, maskname)
        weight = weight_ * mask
        ...
        module.register_parameter(fn.name + "_orig", weight_)
...
class SpectralNormConvLoadStateDictPreHook(object):
    ...
    def __call__(self, state_dict, prefix, local_metadata, strict,
        ...
        fn = self.fn
        version = local_metadata.get('masked_spectral_norm_conv', {}).get(
...
class SpectralNormConvStateDictHook(object):
    ...
    def __call__(self, module, state_dict, prefix, local_metadata):
        if 'masked_spectral_norm_conv' not in local_metadata:
            local_metadata['masked_spectral_norm_conv'] = {}
        key = self.fn.name + '.version'
        if key in local_metadata['masked_spectral_norm_conv']:
            raise RuntimeError(
                "Unexpected key in metadata['masked_spectral_norm_conv']: {}".format(
                    key))
        local_metadata['masked_spectral_norm_conv'][key] = self.fn._version


def masked_spectral_norm_conv(module, coeff, input_dim, name='weight', maskname=None,
    ...
    MaskedSpectralNormConv.apply(
        module, coeff, input_dim_4d, name, maskname,
        n_power_iterations, eps)
    return module


def remove_masked_spectral_norm_conv(module, name='weight'):
    ...
    """
    for k, hook in module._forward_pre_hooks.items():
        if isinstance(hook, MaskedSpectralNormConv) and hook.name == name:
            hook.remove(module)
