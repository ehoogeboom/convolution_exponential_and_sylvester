...
from models.transformations.iresnet.conv2d import get_conv2d
...
class ResBlockWrapper(BaseTransformation):
    def __init__(self, input_size, n_channels):
        super(ResBlockWrapper, self).__init__()

        self.iresblock_module = _resblock(input_size, n_channels)

    def forward(self, x, logdet, context, reverse=False):
        assert context is None

        if not reverse:
            z, logdet = self.iresblock_module(x, logdet)
        else:
            z, logdet = self.iresblock_module.inverse(x, logdet)
...
def _resblock(initial_size, idim):

    # Taken from default args from https://github.com/rtqichen/residual-flows/
    learn_p = False
    ...
    nnet.append(
        get_conv2d(
            ...
            domain=_domains[0], codomain=_codomains[0], atol=sn_atol,
            rtol=sn_rtol
        )
    )
    ...
    for i, k in enumerate(ks[1:-1]):
        nnet.append(
            get_conv2d(
                ...
                n_iterations=n_lipschitz_iters,
                domain=_domains[i + 1], codomain=_codomains[i + 1],
    ...
    nnet.append(
        get_conv2d(
            ...
            n_iterations=n_lipschitz_iters, domain=_domains[-1],
            codomain=_codomains[-1], atol=sn_atol, rtol=sn_rtol
        )
    )
...
class iResBlock(nn.Module):
    ...
    def inverse(self, y, logpy=None):
        ...
        if logpy is None:
            ...
            return x, logpy - self._logdetgrad(x)[1].squeeze()
...
if __name__ == '__main__':
    ...
    layer = ResBlockWrapper(input_size, 512)

    print(layer)

    z, _ = layer(x, ldj, None)

    x_recon, _ = layer(z, ldj, None, reverse=True)
